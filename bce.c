		...
		broadcast_system_t *sys)
{
	...
	char buf[1024];
	size_t count = fread(buf, 1, 1024, params);
	if (!count) pbc_die("input error");
	if (pairing_init_set_buf(p->pairing, buf, count)) pbc_die("pairing init failed");
	fclose(params);
		...
		int num_users, char *pairFileName)
{
	...
	char s[4096];
	size_t count = fread(s, 1, 4096, curveFile);
	if (!count) pbc_die("input error");
    printf("\nFile read into arr\n");
	if (pairing_init_set_buf(gbs->pairing, s, count)) pbc_die("pairing init failed");
    printf("PAIRING");
	fclose(curveFile);
